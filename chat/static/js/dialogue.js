...
function mark_as_read() {
    ...
    setTimeout(function() {
        if (globWebSocket.readyState === WebSocket.OPEN) {
            ...
            elements.forEach(function(element) {
                ...
                if (parent) {
                    ...
                    if (parent.dataset.sender != playerId) {
                        // Отправляем в сокет сообщение, что мы его прочли
                        globWebSocket.send(JSON.stringify({
                            'message': 'was_read',
            ...
            // Если соединение еще не установлено, повторно вызываем mark_as_read
            mark_as_read();
        }
    }, 1000);
}
