    ...
    if request.method == "POST":
        ...
        for link in links_list:
            ...
            if not Neighbours.objects.filter(
                    Q(region_1=all_regions.get(pk=int(link[0])), region_2=all_regions.get(pk=int(link[1])))
                    | Q(region_2=all_regions.get(pk=int(link[0])), region_1=all_regions.get(pk=int(link[1])))
            ).exists():
                neig = Neighbours(
                    region_1=all_regions.get(pk=int(link[0])),
                    region_2=all_regions.get(pk=int(link[1])),
                )
                neig.save()
