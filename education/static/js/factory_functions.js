...
function set_component_line(schema, crude, last_crude_name){
    ...
    var max_value = 0;
    if (crude=='energy'){
        max_value = Math.floor(100/schema[crude]) * schema[crude]
    }
    else{
...
function decreaseStocks(i) {
  if (i == 0) return; // Остановка, когда счётчик достигнет 0

  setInputValue(i);
  // Получаем все элементы с классом factory__blockElement
  const factoryBlocks = document.querySelectorAll('.factory__blockElement');

  factoryBlocks.forEach((block) => {
    // Находим элемент с классом storage_stocks внутри factory__blockElement
    const storageStocks = block.querySelector('.storage_stocks');
    // Находим элемент с классом factory__range внутри factory__blockElement
    const factoryRange = block.querySelector('.factory__range');

    // Если оба элемента существуют и storage_stocks содержит числовое значение
    if (storageStocks && factoryRange) {
      const step = parseInt(factoryRange.getAttribute('step')) || 0;
      const currentStocks = parseInt(storageStocks.textContent.replace(/\s+/g, '')) || 0;

      // Уменьшаем значение storage_stocks на величину step
      storageStocks.textContent = (currentStocks - step);
    }
  });

  // Вызываем decreaseStocks для следующей итерации с задержкой 10 миллисекунд
  setTimeout(() => decreaseStocks(i - 1), 10); // 10 миллисекунд задержки
}

function edu_produce(){
