...
def multiple_sum(sum, lag=0):
    ...
    today = timezone.now()
    weeks_passed = (
                               today - start_date).days // 7 + 1 + lag  # Полные недели с начальной даты. Если задан lag, то рост (выплат) будет сильнее

    # Рассчитываем добавку
    if today >= start_date:
        bonus_percentage = min(weeks_passed * 125, 1000)  # Ограничиваем до 1000%
        if type(sum) == Decimal:
            sum *= Decimal(bonus_percentage / 100)  # Применяем добавку
        else:
