...
def black_friday(request):
    ...
    if GameEvent.objects.filter(running=True, type='fd', event_start__lt=timezone.now(), event_end__gt=timezone.now()).exists():

        event = GameEvent.objects.filter(running=True, type='fd', event_start__lt=timezone.now(), event_end__gt=timezone.now()).first()
    ...
    page = 'event/black_friday.html'
    # отправляем в форму
    response = render(request, page, {
        'page_name': pgettext("game_event", 'Неделя чёрной пятницы!'),
